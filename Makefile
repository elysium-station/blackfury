...
SIMAPP = ./app
HTTPS_GIT := https://github.com/elysium-station/blackfury.git
DOCKER := $(shell which docker)
DOCKER_BUF := $(DOCKER) run --rm -v $(CURDIR):/workspace --workdir /workspace bufbuild/buf
NAMESPACE := blackfuryzone
PROJECT := blackfury
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/elysium-station/blackfury/types"
	godoc -http=:6060
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/elysium-station/blackfury
.PHONY: format
...
localnet-start: localnet-stop build-linux localnet-build
	...
	then docker run --rm -v $(BUILDDIR):/blackfury:Z blackfuryzone/localnetnode testnet init-files -v 4 -o /blackfury --starting-ip-address 192.168.10.2 --predetermined-mnemonic --keyring-backend=test; \
	fi
	docker-compose up -d
...
bridging-localnet-start: bridging-localnet-stop build-linux localnet-build
	...
	then docker run --rm -v $(BUILD_BRIDGING_DIR):/blackfury:Z blackfuryzone/localnetnode testnet init-files -v 4 -o /blackfury --starting-ip-address 192.168.11.2 --predetermined-mnemonic --keyring-backend=test; \
	fi
...
PACKAGE_NAME:=github.com/elysium-station/blackfury
